...
class _TodolistState extends State<Todolist> {
  ...
  void _filterTodos() {
    ...
    setState(() {
      filteredTodoList =
          todoList.where((todo) {
            return todo.listTitle.toLowerCase().contains(query) ||
                todo.listDesc.toLowerCase().contains(query);
          }).toList();
    });
  }
                ...
                Expanded(
                  child:
                      noData
                          ...
                          ? Center(
                            // ถ้า filteredTodoList ว่างและมีการพิมพ์คำค้นหา
                            child: Text(
                              "ไม่พบข้อมูล", // ข้อความแจ้งว่าหากไม่พบข้อมูล
                              style: TextStyle(
                                fontSize: 16,
                                color: Colors.grey,
                              ),
                            ...
                            child: ListView.builder(
                              itemCount:
                                  filteredTodoList.isEmpty &&
                                      ...
                                      ? todoList
                                          .length // ถ้าไม่มีการค้นหา ให้แสดงรายการทั้งหมด
                                      : filteredTodoList
                                          .length, // ถ้ามีการค้นหาก็แสดงเฉพาะผลการค้นหา
                              itemBuilder: (context, index) {
                                final todo =
                                    filteredTodoList.isEmpty &&
                                            searchController.text.isEmpty
                                        ? todoList[index] // ถ้าไม่มีการค้นหา ให้ใช้ todoList
                                        : filteredTodoList[index]; // ถ้ามีการค้นหา ให้ใช้ filteredTodoList
                                DateTime utcDateTime = todo.lastUpdate.toUtc();
                                DateTime thailandTime = utcDateTime.add(
                                  Duration(hours: 7),
                                );
